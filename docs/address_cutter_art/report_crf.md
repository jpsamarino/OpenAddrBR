...
- **spaCy CRF (NER)**: O otimizador foca na eficiência, com hiperparâmetros padrões adequados. Foi utilizada a inicialização de vetores pré-treinados estáticos gerados pelo FastText. (Nota de execução: em determinados ambientes Windows, o limite do MAX_PATH requer configurações específicas para o carregamento correto da biblioteca spaCy).
